...
class WrappedInt {
	...
	int inverse(int p, int k) {
		...
		if(!rWasOne) {
			throw new Exception("No inverse for " ~ to!(string)(p) ~ " (mod " ~ to!(string)(k) ~ ")");
		}
	...
	WrappedInt opAdd(WrappedInt o) {
		if(k != o.k) {
			throw new Exception("Range " ~ to!(string)(o.k) ~ " does not match expected range " ~ to!(string)(k));
		}
	...
	WrappedInt[] sqrt() {
		...
		if(numRoots == 0) {
			throw new Exception("No square root exists for " ~ to!(string)(val) ~ " (mod " ~ to!(string)(k) ~ ")");
		} else if(numRoots == 1) {
			throw new Exception("Expected two roots for " ~ to!(string)(val) ~ ", but only one was found (" ~ to!(string)(res[0].val) ~ ")");
		} else if (numRoots > 2) {
			throw new Exception("Expected two roots for " ~ to!(string)(val) ~ ", but found " ~ to!(string)(numRoots));
		} else {
			return res;
		}
	}
	...
	int opCmp(Object o) {
		...
		if(owi) {
			if(owi.k == k) {
				...
				throw new Exception("Can't compare WrappedInt with order " ~ to!(string)(k) ~ " to one with order " ~ to!(string)(owi.k));
			}
		} else {
